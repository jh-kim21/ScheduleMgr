 ...
 *
 * 이 시점에는(@vue/test-utils/DOM 환경 도입 전) 컴포넌트를 직접 마운트해 검증할 방법이
 * 없었으므로, 리더가 고정한 동작 계약 5가지를 기준으로 "Developer1이 뽑아낼 법한" 순수 함수
 * 시그니처를 가정해 이 스펙을 작성한다. 아래는 실제 구현이 아니라 테스트가 기대하는 계약이다 —
 * 이름이 다르면 리더가 Developer1과 맞춰야 한다. (지금은 `@vue/test-utils`/`happy-dom`으로
 * `WbsTree`를 마운트할 수 있지만, 방향키 이동·더블클릭 가드는 판정이지 렌더링이 아니므로
 * 순수 함수로 고정하는 편이 여전히 더 빠르고 규칙을 더 또렷하게 지킨다 — CLAUDE.md
 * "컴포넌트 테스트".)
 *
