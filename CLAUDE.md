...
- **예외: WBS 폼 안의 체크포인트는 [저장]과 무관하게 즉시 저장됩니다**(`CheckpointList`). 위 일반
  규칙과 달리 추가·승인·삭제가 각자 바로 반영되고 [취소]로 되돌릴 수 없습니다 — 화면에도 그 사실을
  적어 둡니다. 항목 자체의 필드(이름·일정 등)와 체크포인트는 서로 다른 API로 저장되는 별개의
  자원이라, 하나의 [저장] 버튼 뒤로 묶으면 두 저장의 성패가 갈릴 때(체크포인트는 이미 반영됐는데
  항목 저장은 거부됨) 설명하기 더 어렵습니다.
...
- **진척은 Dashboard의 탭**입니다(`/dashboard?tab=progress`). 요약은 읽기 전용 집계이고 진척은 그
  ...
  기존 링크가 살아 있습니다. **체크포인트만 예외입니다** — 추가·승인·삭제는 WBS 항목 편집 폼으로
  옮겼고(실행 방식을 Waterfall/Hybrid로 바꾼 바로 그 자리에서 분모를 채우기 위해서입니다), 진척
  탭의 목록은 그 숫자가 무엇으로 이루어졌는지 보여주는 읽기 전용 자리로 남아 `/wbs?focus=<id>`
  링크로 편집을 안내합니다. **편집 UI는 WBS 폼 한 곳에만** 있습니다 — 두 곳에 만들면 한쪽을 고칠
  때 다른 쪽을 잊습니다.
