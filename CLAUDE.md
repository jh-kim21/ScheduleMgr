...
그 위에 **UI 상용화 정비**를 1~3단계까지 했습니다 — `style.css`에 컨트롤 층을 신설하고, 명암비를
측정해 기준에 맞추고(라이트 16건·다크 7건 미달 → WCAG 예외 2건만 남김), 손으로 만든 대화상자 넷을
`ModalDialog`로 흡수하면서 **중첩 지원**을 넣고, 27개 화면이 컨트롤 층을 쓰게 했습니다. 기능을
더하면서 CSS 번들은 108.4kB → 96.4kB로 줄었습니다. **남은 것은 드래그 전용 조작의 키보드·터치
대안**입니다 — WBS 트리 재정렬·재부모화(`WbsTree`)와 칸반 카드 이동(`SprintBoard`)이 아직
HTML5 네이티브 DnD 전용이라 키보드로도 터치 기기에서도 쓸 수 없습니다(WCAG 2.5.7). 그 밖에
`aria-live`가 저장소 전체에 0건이고, 폼 제출 중 스피너·잠금이 없으며, 반응형 breakpoint를 가진
화면이 52개 중 5개입니다.
...
- **CORS 허용 오리진은 프로필 설정값**(`project-flow.cors.allowed-origin-patterns`, 쉼표 구분)이고
  ...
  `desktop`은 루프백 전 포트(`http://localhost:[*]`, `http://127.0.0.1:[*]`)에 더해 호스팅 서버
  IP(`http://192.168.60.70:[*]`)까지 허용하고, `server`는 `CORS_ALLOWED_ORIGINS`가 없으면 아무
  교차 오리진도 허용하지 않습니다.
  - **포트를 하나로 고정하지 마세요.** 브라우저는 same-origin이라도 GET 이외의 메서드에는 `Origin`을
    ...
    호스팅 서버 IP도 같은 이유로 포트 부분은 `[*]`로 열어 둡니다.
  - **IP는 사용자 결정으로 하나만 고정했습니다.** 사설 대역(`192.168.*.*`, `10.*.*.*`) 전체를 열지
    않고 실제 호스팅 서버 IP 하나만 허용하기로 했습니다 — 그래서 이 서버의 주소가 바뀌면
    `application-desktop.yml`의 이 줄을 고치거나 `CORS_ALLOWED_ORIGINS`로 덮어야 합니다. 대역 전체를
    여는 것보다 좁은 쪽을 택한 비용입니다.
  - **`*`로 아무 오리진이나 허용하지 마세요.** 그러면 사용자가 악성 웹사이트를 여는 것만으로 그
    사이트의 스크립트가 `http://localhost:8081/api/...`에 쓰기 요청을 보낼 수 있습니다 — 브라우저가
    막아 주던 것을 우리가 열어 주는 셈입니다. 루프백·지정한 서버 주소로 좁혀 둔 이유가 그것이고,
    그 밖의 주소는 계속 403이어야 합니다. `https://`·호스트명 접속이 필요해지면
    `CORS_ALLOWED_ORIGINS`로 덮습니다.
...
- **체크포인트(Waterfall·Hybrid의 분모)의 정의·수정·삭제·승인은 WBS 트리에서 합니다** — Work
  ...
  대화상자 안에 목록을 겹쳐 넣으면 폼의 [저장/취소]와 체크포인트의 즉시 저장이 한 화면에서
  엇갈립니다.
...
- **모든 추가·수정 폼은 대화상자**입니다([`ModalDialog`](frontend/src/components/ModalDialog.vue)).
  고정 패널로 두면 표를 화면 아래로 밀어내는데, 이 앱의 화면은 대부분 *읽는* 것이 주된 행위입니다.
  화면마다 `formOpen` 같은 상태와 `＋ … 추가` 버튼을 두고, 폼 컴포넌트가 자기 `title`을 계산해
  `ModalDialog`에 넘깁니다 — 뷰가 제목을 만들면 "하위 항목 추가 — 1.2 화면 설계"처럼 문맥이 붙는
  제목이 두 곳에서 중복됩니다. `MemberEditor`가 같은 이유로 자기 제목(`구성원 관리 — {프로젝트명}`)을
  계산해 스스로를 `ModalDialog`로 감쌉니다 — 프로젝트 화면은 상세 패널이 없는 목록 화면이라, 전역
  `selectedProjectId`가 아니라 행에서 방금 누른 프로젝트를 기억해 둡니다.
- **폼에 패널 크롬(테두리·패딩·여백)을 두지 마세요.** 대화상자가 갖습니다. 폼은 필드만 담습니다.
- **저장 거부 메시지를 대화상자 안에 보여 줍니다**(`:error`). 화면 본문의 `<p class="error">`는
  대화상자 뒤에 가려지므로, 그것만 있으면 저장이 조용히 실패한 것처럼 보입니다. 그래서 뷰의 저장
  핸들러는 **성공했을 때만 닫습니다** — `useWbs` 등의 변경 함수가 boolean 을 돌려주는 이유입니다.
  거부되면 입력값이 그대로 남아 있어야 합니다.
  - 실제로 `ProgressPanel`의 기준선 승인이 이 결함을 그대로 갖고 있었습니다 — 오류가 화면 본문에
    렌더되는데 그 자리가 손수 만든 오버레이(`z-index: 50`)에 완전히 덮여, **승인이 거부되면 화면에
    아무 일도 일어나지 않았습니다.** `ProgressPanel.spec.ts`가 이 성질을 고정합니다.
  - **폼이 `emit`으로 제출하면 결과를 돌려받을 수 없습니다.** 그런 폼(중첩 대화상자를 자기가 여는
    `MemberEditor` 같은 것)은 제출만 **함수 prop**(`(input) => Promise<boolean>`)으로 받고 `await`
    한 뒤 `true`일 때만 닫습니다. **"목록이 실제로 바뀌었는지"를 watch해서 추측하지 마세요** — 그
    방식은 서버 응답과 클라이언트 입력값의 문자열 정확 일치에 닫힘을 걸게 되어, 서버가 값을 조금
    이라도 다르게 정규화하면 저장은 성공했는데 대화상자가 닫히지도 오류가 뜨지도 않습니다.
    **API 호출은 여전히 뷰가 합니다** — 폼이 직접 부르게 바꾸면 `readOnly` 게이팅 자리가 흩어집니다.
- **예외: WBS 트리에서 Work Package 행을 펼쳐 다루는 체크포인트는 즉시 저장됩니다**(`CheckpointList`
  를 `editable: true`로 띄운 것). 위 일반 규칙과 달리 추가·수정·승인·삭제가 각자 바로 반영되고
...
- **`ModalDialog`는 중첩을 전제로 동작합니다.** 앱이 실제로 겹쳐 씁니다 — `MemberEditor`의
  "구성원 관리" 안 "구성원 추가"가 일상 경로이고, `CommitPanel`은 용량 초과 시 히스토리 → 폼 →
  용량 대화상자로 **3중첩**까지 갑니다. 그래서 파일 안에 일반 `<script>` 블록으로 **모듈 스코프
  스택**을 두고 세 가지를 스택 기준으로 처리합니다. **단순화하지 마세요** — 예전에 인스턴스마다
  독립적으로 처리하다가 전부 깨져 있었습니다.
  - **Escape·Tab 트랩은 최상단 하나만 반응**합니다(핸들러 맨 앞의 `isTopDialog`). 이 한 줄이
    "Escape가 열린 대화상자를 전부 닫는" 문제와 "바깥이 안쪽 패널의 포커스를 보고 자기 Tab 트랩을
    오작동시키는" 문제를 동시에 막습니다. 리스너를 인스턴스마다 거는 것 자체는 문제가 아닙니다.
  - **배경 스크롤 잠금은 참조 카운트**이고 원래 `overflow` 값을 저장했다 복원합니다. `''`로
    하드코딩해 되돌리면 안쪽이 닫힐 때 바깥이 열려 있는데도 스크롤이 살아나고, 다른 코드가
    설정해 둔 값도 지웁니다.
  - **배경으로 물러난 대화상자에는 `inert`+`aria-hidden`** 을 겁니다(자기 자신에게는 절대 걸지
    않습니다 — 포커스를 잃습니다). 시각적 트랩만으로는 스크린리더 가상 커서가 뒤로 통과합니다.
  - **`[autofocus]` 요소가 있으면 초기 포커스가 거기로** 갑니다(없으면 첫 입력 칸 →
    `focusable()[0]`). 입력이 없는 **파괴적 확인 대화상자는 취소 버튼에 `autofocus`** 를 둡니다 —
    안 그러면 열자마자 반사적으로 Enter를 눌러 실행됩니다.
- **`role="dialog"`를 직접 쓰지 마세요.** 저장소에서 이 속성을 쓰는 곳은 `ModalDialog` 하나여야
  합니다. 예전에 손으로 만든 대화상자가 넷 있었는데(`ProgressPanel`의 기준선 승인, `SprintBoard`의
  완료 확인·차단 표시, `SprintView`의 Sprint 종료) 전부 포커스 트랩·Escape·접근 가능한 이름이
  없었고, `role="dialog"`만 붙어 있어 오히려 **이름 없는 대화상자**로 안내됐습니다. `SprintView`는
  같은 파일 안에서 "시작 취소"는 `ModalDialog`를 쓰고 "종료"만 직접 만들고 있었습니다 — 규칙을
  모르는 게 아니라 한 곳에서 새어나간 것입니다.
- **표는 셀을 줄바꿈하지 않고 가로 스크롤로 넘깁니다.** 전역 유틸리티
  [`.table-scroll`](frontend/src/style.css)로 감싸고 `th, td`에 `white-space: nowrap`을 둡니다.
  값이 세로로 접히면 행 높이가 들쭉날쭉해져 훑어 읽기 어렵습니다. 설명·비고처럼 자유 입력 칸만
  `.cell-clip`으로 폭 상한을 두고 `title`로 전체를 보여 줍니다 — 긴 메모 하나가 표를 화면 몇 배로
  늘리지 않게 하려는 것입니다.
  - **`v-else`가 붙은 표를 감쌀 때는 `v-else`를 래퍼로 옮기세요.** 사이에 `div`가 끼면 `v-if`와
    인접하지 않아 컴파일이 실패합니다(BacklogList 에서 실제로 겪었습니다).
- **본문 폭 상한은 1600px**입니다([`App.vue`](frontend/src/App.vue)). 표와 간트가 주된 내용이라
...
- **되돌릴 수 없는 동작에는 예외 없이 `confirm()`을 거칩니다.** 프로젝트·WBS 항목·구성원·분야·
  RAID 항목·커밋·Backlog 항목·Sprint·Sprint 배정·체크포인트 삭제와 일정 재계산이 전부 여기
  해당합니다. 한때 Backlog 항목 삭제·Sprint 배정 제거·체크포인트 삭제 셋만 빠져 있었는데, 규칙이
  없었던 게 아니라 **세 곳이 규칙에서 새어나간 것**이었습니다. **부작용이 있으면 문구에 담으세요** —
  구성원 삭제는 RACI 배정이 함께 지워지는 것을, 승인된 체크포인트 삭제는 그 Work Package의 진척
  숫자가 즉시 바뀌는 것을(Waterfall·Hybrid의 분모이므로) 알립니다.
  - `disabled` 가드와 확인 절차는 **다른 층**입니다. 확인은 그 위에 얹는 것이지 대체가 아닙니다 —
    예컨대 Sprint 삭제는 `canDelete`로 배정 있는 Sprint를 먼저 막고, 그다음 확인을 받습니다.
- **커밋 조회 중에는 전 화면에 고정 배너가 뜨고 쓰기 진입점이 플래그 하나로 잠깁니다.**
  ...
  확인할 수 있습니다.
  - **진입점을 막았어도 핸들러에서 한 번 더 막습니다.** `ProjectsView`의 구성원 추가·수정·삭제
    핸들러 셋이 이 이중 확인에서 빠져 있었는데, 같은 파일의 다른 핸들러는 전부 하고 있었습니다.
    당시 그것을 가려 주던 것은 대화상자 백드롭뿐이었고, **그 방패는 `ModalDialog` 중첩 결함으로
    깨질 수 있었습니다** — 우연한 방어에 기대지 마세요.
  - **읽기 동작은 잠그지 않습니다.** WBS의 필터·열 설정, 커밋 목록 열기는 커밋 조회 중에도
    활성입니다 — 보는 것은 쓰기가 아닙니다.

### 컨트롤 층 (프론트엔드)

[`style.css`](frontend/src/style.css)에는 **색 토큰 층**과 **컨트롤 층**이 함께 있습니다. 예전에는
토큰 층만 있어서 파일마다 버튼을 처음부터 그렸고, 그때마다 hover·focus·disabled·타깃 크기를 사람이
기억해서 넣어야 했습니다. 그 결과 `<button>`을 쓰는 34개 파일 중 **20개에 hover가 없었고**,
`border-radius`가 **9종**으로 흩어졌으며, 타깃 크기 미달 컨트롤이 여럿이었습니다. 색 규칙은 엄격히
지켜졌는데 형태 규칙만 아무도 강제하지 않은 상태였습니다.

- **새 화면에서 버튼·입력을 처음부터 그리지 마세요.** `button`·`input`·`select`·`textarea`는 요소
  선택자로 기본이 깔리고, 변형은 `.primary` / `.tonal` / `.ghost` / `.danger` / `.link` / `.btn-icon`
  입니다. 전역이 주는 것(패딩·radius·테두리·hover 상태 레이어·`:focus-visible` 링·`:disabled`·
  24×24px 최소 타깃·`touch-action`)을 로컬에 다시 적지 마세요.
- **변형 선택자는 반드시 클래스 단독(`.primary`)이고 `button.primary`가 아닙니다.** 요소를 붙이면
  특이도가 (0,1,1)이 되어 컴포넌트의 scoped `button {}`(`button[data-v-x]`, 역시 (0,1,1))과
  **동점**이 되고, 어느 쪽이 이기는지가 번들러의 CSS 출력 순서에 달리게 됩니다. 그 조합에 걸리는
  파일이 17개입니다. 클래스 단독이면 (0,1,0)이라 scoped 로컬이 **특이도로** 항상 이깁니다 —
  "읽기에는 `button.primary`가 명확한데" 하고 되돌리지 마세요.
- **그래서 이 층은 "덮어쓰기"가 아니라 "빈 자리 채우기"입니다.** 이미 자기 스타일을 가진 파일은
  그대로 이기고, 스타일이 없던 파일만 기본값을 받습니다. 채택은 각 파일의 로컬 `<style scoped>`에서
  **중복 블록을 지우는 것**이지 클래스를 새로 붙이는 것이 아닙니다(변형 이름을 기존에 쓰이던 것
  그대로 전역 승격했으므로 템플릿은 고칠 필요가 없습니다).
- **로컬 버튼 규칙을 지울 때 `background`와 `color`는 짝으로 다루세요.** 하나만 지우면 그 버튼 안의
  자식(`<span>`·`<svg>`)이 상속하던 색이 고아가 됩니다. 실제로 `SprintView`의 Sprint 목록 이름이
  로컬 `button{}`의 `color: var(--accent-fg)`를 상속하고 있었는데 배경은 `--surface`여서, **라이트는
  흰 배경에 흰 글자, 다크는 어두운 배경에 어두운 글자**로 양쪽 테마 모두 사실상 보이지 않았습니다
  (한쪽만 깨졌다면 진작 눈에 띄었을 것입니다).
- **`label`·`.name`·`.actions` 같은 흔한 이름을 bare 요소 선택자로 전역화할 때는** 그 파일 안에 같은
  이름의 다른 용도가 있는지 먼저 보세요 — `SprintView`에서 `label {}`로 올렸다가 `.project-picker`
  레이아웃을 깨뜨릴 뻔했습니다.
- **`--scrim`은 대화상자 배경 전용 토큰**입니다. 예전에는 `ModalDialog`가 55%, 손으로 만든 셋이
  45%를 하드코딩해 대화상자를 여는 화면마다 배경이 다르게 어두워졌습니다.
...
- **`confirm()`은 `vi.spyOn`이 아니라 `vi.stubGlobal`로 대신합니다.** happy-dom이 `window.confirm`을
  구현하지 않아 스파이를 걸 대상 함수 자체가 없습니다. 정리는 **`afterEach`의 `vi.unstubAllGlobals()`**
  가 하며, `vi.restoreAllMocks`는 `stubGlobal`을 되돌리지 못하니 그것으로 바꾸지 마세요 — 안 그러면
  스텁이 다음 테스트 파일로 샙니다([`CheckpointList.spec.ts`](frontend/src/features/progress/CheckpointList.spec.ts)가
  예시입니다).
- **"거부되면 대화상자가 열린 채 입력값이 남는다"는 테스트로 고정하세요.** 소스 리뷰로는 놓치기
  쉽고 실제로 놓쳤습니다 — 오류가 대화상자 뒤에 가려져 저장 실패가 보이지 않던 결함이 있었습니다.
  `ProgressPanel.spec.ts`(거부 시 대화상자 잔존 + `[role="alert"]` 내용 + 입력값 보존, 성공 케이스를
  대조군으로)와 `MemberEditor.spec.ts`(제출 prop이 `false`를 돌려줄 때)가 그 형태입니다.
...
- **토큰 색을 바꿀 때는 눈으로 판단하지 말고 명암비를 재세요.** 본문 글자 4.5:1(WCAG 1.4.3), 입력
  필드 경계·차트 채움 같은 UI 요소 3:1(1.4.11)이 기준이고, **라이트·다크 양쪽 다** 봐야 합니다.
  한 번 잰 결과 라이트 16건·다크 7건이 미달이었는데, 그중 `--text-faint`(2.85:1)는 32개 파일에서
  실제 본문으로 쓰이고 있었습니다. 지금은 아래 두 가지만 남겨 두었습니다.
  - **`--disabled-fg`/`--disabled-bg`(2.08)는 예외입니다** — WCAG가 비활성 컨트롤을 명시적으로
    제외합니다. 4.5:1까지 억지로 올리면 비활성이 활성처럼 보입니다.
  - **`--border`(1.27)도 예외입니다** — 순수 장식 구분선이라 3:1 대상이 아닙니다. 입력 필드
    경계인 **`--border-input`만** 기준을 지킵니다.
- **막대·배지에 쓰는 `--status-*`는 두 제약을 동시에 만족해야 합니다**: 그 위의 글자(`--status-fg`)가
  4.5:1이고, 채움 자체가 배경 대비 3:1입니다. 간트 막대·지연 배지·RAID 노출도 배지가 같은 값을
  공유하므로 하나를 바꾸면 셋이 함께 움직입니다. `not-started`와 `unscheduled`는 원래 비슷한
  회색이라 기준을 맞추면서 **색조를 냉/난으로 갈라** 구분을 살렸습니다.

### RACI 설계상 알아둘 점

- **구성원(요구사항 4.2)이 매트릭스의 열입니다.** RACI는 (업무 × 사람) 표라서 사람 없이는 표가
  성립하지 않습니다. 그래서 RACI를 하려면 구성원 관리가 먼저 필요합니다. **구성원 추가·수정·삭제는
  프로젝트 화면(행의 `구성원` 버튼)에서 하고, RACI 화면은 매트릭스 응답의 `members`를 열로 읽기만
  합니다**(Step 7) — 구성원은 프로젝트 스코프 개념이라 RACI에 종속될 이유가 없습니다.
...
- **구성원 API만 예외로 구성원 하나만 반환**합니다(요구사항 4.2는 프로젝트 기능이라 RACI를 몰라야
  합니다). **RACI 화면이 구성원 변경 후 다시 읽는 것이 아닙니다** — 구성원은 Step 7부터 프로젝트
...
- **RACI 캐시 키에는 날짜가 들어가지 않습니다**(`raciCacheKeyFor`). 행은 WBS에서 오지만 "오늘"을
  ...
  들어가고**, Step 7부터는 열(구성원) 자체가 바뀔 수 있어 **멤버 리비전도 들어갑니다.**
...
- **RAID 캐시 키는 `(프로젝트, WBS·Backlog·Sprint·멤버 리비전, 로컬 날짜)`입니다.** 앞의 세 리비전이
  들어가는 이유는 위 연결 때문입니다 — 항목이 WBS 업무·Sprint·Backlog 어디에나 붙고, 등록부가 그
  대상의 이름을 보여주므로 어느 쪽이 이동·개명돼도 낡습니다. 멤버 리비전은 소유자
  이름(`ownerName`)이 프로젝트 구성원에서 오기 때문입니다(Step 7). 로컬 날짜는 기한 초과가 날짜
  기준이라 필요합니다(표시하는 기준일은 항상 서버의 `referenceDate`).
...
- **진척은 Dashboard의 탭**입니다(`/dashboard?tab=progress`). 요약은 읽기 전용 집계이고 진척은 그
  숫자의 *근거를 입력하는* 곳(가중치·기준선·스냅샷)이라 한 화면의 두 면으로 두었습니다. 이렇게
  해야 최상위 메뉴가 설계서의 일곱 개로 떨어집니다. 예전 `/progress`는 이 탭으로 리다이렉트하므로
  기존 링크가 살아 있습니다. **체크포인트는 이 탭에 속하지 않습니다** — 정의·수정·삭제·승인이
  전부 **WBS 트리**에서 일어납니다(Work Package 행을 펼치면 그 자리에 나타납니다). 진척을 올리는
  행위는 각자의 작업 화면에서 하고(Agile은 Board/Backlog에서 Story·Bug를 완료로 전이, Waterfall은
  WBS 트리에서 체크포인트를 승인), 진척 탭은 그 결과만 본다는 원칙을 지키기 위해서입니다 — 예전엔
  Waterfall만 진척 탭 안에서 직접 올려 비대칭이었습니다. 진척 탭의 체크포인트 목록은
  [`CheckpointList`](frontend/src/features/progress/CheckpointList.vue)를 `editable: false`로
  띄운 **읽기 전용 조망**입니다(제목·가중치·완료조건·승인 상태만 표시, 버튼 없음). WBS 트리도 같은
  컴포넌트를 `editable: true`로 씁니다 — 추가·수정·삭제·승인 UI는 이 한 곳에만 있습니다(두 곳에
  만들면 한쪽을 고칠 때 다른 쪽을 잊습니다). 진척 탭에는 `/wbs?focus=<id>` 링크로 그 트리 행을
  찾아가는 길을 안내합니다. **기준선 승인과 보고 스냅샷 저장은 진척 탭에 남아 있습니다** — Work
  Package 단위 실행이 아니라 진척 숫자를 보면서 내리는 프로젝트 레벨 판단이기 때문입니다.
  - 체크포인트는 **데이터 모델을 바꾸지 않고** 화면에서만 Work Package의 자식처럼 그립니다.
    `acceptance_checkpoints`는 그대로 별도 테이블입니다 — 진짜 WBS 자식으로 만들면 "Work
    Package에는 하위를 둘 수 없다"는 규칙, `WbsNode.summary()`의 자식 유무 판정(체크포인트를
    하나 추가하는 순간 그 Work Package의 일정이 하위 파생 값으로 바뀌어 입력한 날짜가 사라집니다),
    진척 계산 이중화가 전부 깨집니다.
...
- **UI 상용화 정비 4단계 — 드래그 전용 조작의 대안.** WBS 트리 재정렬·재부모화(`WbsTree`)와 칸반
  카드 이동(`SprintBoard`)이 HTML5 네이티브 DnD 전용이라 **키보드로도 터치 기기에서도 쓸 수
  없습니다**(WCAG 2.5.7·2.1.1). WBS 툴바에는 이동 버튼 자체가 없고(`하위 추가`/`수정`/`삭제`/
  `열 설정`/`필터`뿐), 칸반 카드에는 `tabindex`·`role`·키 핸들러가 없습니다. CSS 정리가 아니라
  **새 상호작용 설계**(이동 버튼인지, 잘라내기·붙여넣기인지, 대상 선택 메뉴인지)가 먼저 필요합니다.
- **UI 상용화 정비, 그 밖에 남은 것**
  - **`aria-live`가 저장소 전체에 0건**입니다. 저장 결과·일정 재계산 결과·필터 건수·Board 카드
    이동을 스크린리더가 전혀 받지 못합니다(WCAG 4.1.3).
  - **폼 제출 중 스피너·버튼 잠금이 없습니다.** 느린 네트워크에서 중복 제출이 가능합니다.
  - **ARIA 위젯 시맨틱**: WBS 트리가 커스텀 tree/grid인데 `role="tree"`·`aria-activedescendant`가
    없어 화살표 키로 옮긴 선택이 스크린리더에 전달되지 않습니다. 진행률 바에 `role="progressbar"`,
    필터·칩 토글에 `aria-pressed`도 없습니다.
  - **반응형 breakpoint를 가진 화면이 52개 중 5개**입니다(대시보드·진척·보드뿐). 나머지는 사실상
    데스크톱 전용이고 `.table-scroll`의 가로 스크롤에만 기대고 있습니다.
  - **URL에 화면 상태가 없습니다** — WBS 필터·열 설정, RAID 필터·정렬·검색이 북마크·공유되지
    않습니다. `DashboardView`의 `?tab=` 왕복(뒤로가기까지 처리)이 이미 좋은 선례입니다.
  - 남은 타깃 크기 미달(WBS `.toggle`·`.cp-badge`·필터 칩, RACI 글자, 간트 Sprint 레인),
    `GanttChart`의 하드코딩 `rgb()` 1건, 간트 막대 상세가 마우스 hover 전용인 것.
