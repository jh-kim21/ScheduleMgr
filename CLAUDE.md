...
- **화면별 CSV**는 **사람이 Excel에서 읽기 위한 것**입니다. 그래서 반대로 판정값을 포함하고,
  ...
  기준인지 알 수 있게 합니다.
- **CSV는 서버가 아니라 클라이언트에서 만듭니다**([`csv.ts`](frontend/src/shared/csv.ts),
  [`exportRows.ts`](frontend/src/shared/exportRows.ts)). 내보내기는 "지금 보고 있는 것"이어야
  하는데, RAID 화면의 필터·정렬은 로컬이라 서버는 그것을 모릅니다. 서버가 만들면 필터를 걸어
  둔 사용자에게 조용히 전체 로그를 넘기게 됩니다.
...
- **`formatVersion`을 함께 싣습니다.** 아직 가져오기(import)는 없지만, 나중에 만들 때 어떤
  모양의 파일인지 알아야 합니다.
...
- 데이터 가져오기(import) — 내보내기(JSON)만 있고 되읽는 쪽은 없습니다. `formatVersion`은 그때를
  위해 넣어 두었습니다.
